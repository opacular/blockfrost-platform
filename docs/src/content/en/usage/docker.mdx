...
<Callout>
  The node reaches Blockfrost through an outbound WebSocket connection, so port
  `3000` does not need to be exposed to the public Internet. You only need to
  publish it if you want to reach the local API or Prometheus metrics endpoint
  yourself. If you do publish it, prefer binding to localhost (`-p
  127.0.0.1:3000:3000`); using `-p 3000:3000` publishes on all interfaces
  (address `0.0.0.0`), making the port externally accessible.
</Callout>
